...
class DocumentViewVM: ObservableObject {
    ...
    func appendBlock(user: User, workspace: Workspace, document: Document) {
        do {
            let now = Date.now
            if let index = getLastIndex(user: user, workspace: workspace, document: document) {
                let block = Block(id: UUID(), type: .body, content: "New block", order: index + 1, createdAt: now, modifiedAt: now, document: document)
